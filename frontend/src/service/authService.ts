...
export const userDataGoogle = (data: Token) =>
  axios.post("/auth/register-google", data);
...
export const newPassword = (data: NewPassword) =>
  axios.post("/auth/new-password", data);

export const checkToken = (data: Token) =>
  axios.post("/auth/check-token", data);

export const userDataGoogleLogin = (data: Token) =>
  axios.post("/auth/login-google", data);

export const setTokenInLocalStorage = (token: string) =>
  localStorage.setItem("token-acc", JSON.stringify(token));
...
export const isAdmin = () => {
  const token = localStorage.getItem("token-acc");
  return token && jwt_decode<DecodedToken>(token)?.role === "admin";
};
